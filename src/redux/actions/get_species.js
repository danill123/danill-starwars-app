...
export const get_species = () => {
    return async dispatch => {
        try {
            ...
            dispatch({ type: FETCH_SPECIES, list_result : data_parse['results'], next_uri : PARSING_HTTP_TO_HTTPS(data_parse['next']) })
        } catch (error) {
            // error handling
        }
    }
...
export const next_species_data = (uri) => {
    return async dispatch => {
        try {
            ...
            dispatch({ type: NEXT_FETCH_SPECIES, list_result : data_parse['results'], next_uri : PARSING_HTTP_TO_HTTPS(data_parse['next']) })
        } catch (error) {
            // error handling
        }
    }
