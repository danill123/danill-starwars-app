...
export const get_starships = () => {
    return async dispatch => {
        try {
            ...
            dispatch({ type: FETCH_STARSHIPS, list_result : data_parse['results'], next_uri : data_parse['next'] })
        } catch (error) {
            // error handling
        }
    }
...
export const next_starships_data = (uri) => {
    return async dispatch => {
        try {
            ...
            dispatch({ type: NEXT_FETCH_STARSHIPS, list_result : data_parse['results'], next_uri : data_parse['next'] })
        } catch (error) {
            // error handling
        }
    }
